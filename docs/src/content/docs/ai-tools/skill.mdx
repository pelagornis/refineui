---
title: Skill
description: RefineUI agent skill — routes AI assistants to official docs, llm.txt, and design rules without duplicating content.
---
...
| **2. Documentation routing** | Start from `llm.txt` / `llms.txt`, open leaf pages on design.pelagornis.com/refineui; use MCP when available |
...
| Layer | Role |
|-------|------|
| **Doc routing** | Start from `llm.txt` / `llms.txt`, open leaf pages on design.pelagornis.com/refineui |
...
├── references/
│   ├── design-rules.md           # Token + composition contract (summary)
...
| `references/design-rules.md` | Writing or reviewing UI code |
...
1. Read `https://design.pelagornis.com/refineui/llms.txt` or `https://design.pelagornis.com/refineui/llm.txt`
2. Follow links to Foundations, Components, or Development leaf pages
3. For code generation, combine component docs with [Installation](/development/installation/) and [Theming](/development/theming/)

If MCP is configured, prefer `search_docs` and `get_doc_page` from `@refineui/mcp`.
...
Contributors already have `.cursor/rules/` for design-system constraints. The skill complements those rules for **downstream apps** and agents without repo checkout.

Install the skill in consumer projects; in this monorepo it is available at `skills/refineui/` for review and `skills add` distribution.
