...
<div class="not-content">
  <PropSpec
    client:load
    rows={[
    { prop: "appearance", default: "\"filled\"", type: "\"filled\" | \"ghost\" | \"plain\"", description: "filled: field chrome. ghost: gap + radius without border (Calendar caption). plain: flush shell for composed surfaces (Footer locale)." }
    ]}
  />
</div>
