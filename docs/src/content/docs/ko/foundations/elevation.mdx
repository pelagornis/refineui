...
각 Foundation 스텝(`--refineui-elevation-N`)은 활성 테마에 따라 라이트·다크 값을 자동으로 전환합니다. 컴포넌트는 Foundation 스텝에 연결된 아래 의도형 alias를 우선 사용하세요. bare 변수를 사용하세요 — 명시적 `*light` / `*dark` 접미사는 테마와 무관하게 한 모드로 고정해야 할 때만 필요합니다.
