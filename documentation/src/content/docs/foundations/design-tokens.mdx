...
Tokens are the single source for visual values. Foundation variables in Figma map to `@refineui/tokens`, then to CSS variables and Tailwind utilities. Components and product UI should reference **semantic names**, not palette hex or raw `px`.
